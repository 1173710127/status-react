...
(defn- base-list-props
  [{:keys [key-fn render-fn empty-component header footer separator default-separator? pre-binded-header pre-binded-footer]}]
  (let [separator (or separator (when (and platform/ios? default-separator?) default-separator))]
    (reagent/with-let [binded-header (fn [] (reagent/as-element pre-binded-header))
                       binded-footer (fn [] (reagent/as-element pre-binded-footer))]
      (merge (when key-fn            {:keyExtractor (wrap-key-fn key-fn)})
             ...
             (when pre-binded-header {:ListHeaderComponent binded-header})
             (when pre-binded-footer {:ListFooterComponent binded-footer})
